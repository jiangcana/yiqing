...
const install = function (Vue) {
  // Object对象的方法，ES5提供的：defineProperties
  Object.defineProperties(Vue.prototype, {
    $charts: {
      get () {
        return {
          chinaMap: function (id, data) {
            const myEChart = echarts.init(document.getElementById(id))
            const option = {
              // 提示信息
              tooltip: {
                ...
                formatter (data) {
                  return "<a href='/#/city/" + data.name + "' style='color:#fff'><div><p>" + data.name + '</p><p>当前确诊:' + data.value + '</p></div></a>'
                }
              },
              visualMap: [{ // 映射-颜色值
                ...
                pieces: [ // 配置颜色区间
                  ...
                  { min: 500, max: 1000000, color: '#4F070D' }
                ]
              }],
              series: [{
                name: '省',
          ...
          provinceMap (id, cityName, data) {
            ...
            const option = {
              tooltip: {
                formatter (data) {
                  return '<div><p>' + data.name + '</p><p>当前确诊:' + data.value + '</p></div>'
                }
              },
              visualMap: [{ // 映射-颜色值
                orient: 'vertical', // 分段方向:horizontal水平
